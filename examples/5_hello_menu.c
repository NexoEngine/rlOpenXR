...
ShapeType selectedShape = SHAPE_SPHERE;
Color cubeColor;
Color sphereColor;
...
Sphere placedSpheres[MAX_CUBES];
int cubeCount = 0;
int sphereCount = 0;
...
int main()
{
    ...
    Model sphere_model = LoadModelFromMesh(GenMeshSphere(0.2f, 32, 32));
    ...
    while (!WindowShouldClose())        // Detect window close button or ESC key
    {
        ...
        if (get_action_clicked_this_frame(bindings.place_cube_action, bindings.hand_sub_paths[RLOPENXR_HAND_RIGHT]) && cubeCount < MAX_CUBES && sphereCount < MAX_CUBES && !is_left_menu_active)
        {
            ...
            {
                placedSpheres[sphereCount++] = (Sphere){ place_position };
            }
        }
        ...
        if (rlOpenXRBegin() || rlOpenXRBeginMockHMD()) // Render to OpenXR backbuffer
        {
            ...
            BeginMode3D(world_camera);
                ...
                if (selectedShape == SHAPE_CUBE) {
                    ...
                    DrawModelEx(sphere_model, right_hand.position, right_hand_axis, right_hand_angle * RAD2DEG, Vector3One(), PINK);
                }
                ...
                {
                    DrawSphere(placedSpheres[i].position, 0.25f, RED); // Example size for sphere
                }
                ...
                if (is_left_menu_active)
                {
                    ...
                    // Define the bounding box for the menu
                    BoundingBox menuBB = (BoundingBox){
                        (Vector3){menu_position.x - 0.25f, menu_position.y - 0.25f, menu_position.z - 0.005f},
                        (Vector3){menu_position.x + 0.25f, menu_position.y + 0.25f, menu_position.z + 0.005f}
                    };

                    // Adjust the ray to point forward from the right hand
                    Vector3 hand_forward_right = Vector3RotateByQuaternion((Vector3){0, -1, 0}, right_hand.orientation); // Adjust the direction vector to point forward
                    Ray beam_ray = { right_hand.position, hand_forward_right };

                    // Check for collision with the menu
                    Color menuColor = GRAY;
                    if (CheckCollisionRayBox(beam_ray, menuBB))
                    {
                        menuColor = LIGHTGRAY;
                    }
                    ...
                    rlRotatef(180, 0, 1, 0);
                    rlRotatef(90, 1, 0, 0);
                    DrawCube((Vector3){0, 0, 0}, 0.5f, 0.5f, 0.01f, menuColor); // Draw the menu

                    // Draw additional shapes on the menu
                    float shape_offset = 0.1f; // Offset to place shapes side by side

                    // Define the positions of the shapes in the menu's local space
                    Vector3 local_cube_pos = { 0.0f, 0.0f, 0.0f };
                    Vector3 local_sphere_pos = { 0.0f, 0.0f, 0.0f };

                    // Convert local positions to world positions
                    Vector3 global_cube_pos = Vector3Transform(local_cube_pos, menu_transform);
                    Vector3 global_sphere_pos = Vector3Transform(local_sphere_pos, menu_transform);

                    // Draw the shapes in local space
                    if (selectedShape == SHAPE_CUBE) {
                        DrawCube(local_cube_pos, 0.1f, 0.1f, 0.1f, cubeColor);
                    } else if (selectedShape == SHAPE_SPHERE) {
                        DrawSphere(local_sphere_pos, 0.1f, sphereColor);
                    }
                    rlPopMatrix();
